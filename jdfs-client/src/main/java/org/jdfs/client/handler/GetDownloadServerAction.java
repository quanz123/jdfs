 ...
 */
public class GetDownloadServerAction extends AbstractServerAction {
	private long id;
	...
	public GetDownloadServerAction() {
		super();
	}
	 ...
	 */
	public GetDownloadServerAction(long id, SocketAddress trackerAddress,
	...
	protected Iterator<JdfsRequest> getRequestIterator(Object request,
		...
		GetDownloadServerRequest req = new GetDownloadServerRequest();
		req.setId(id);
	...
	protected Object createResponse(Object request,
		...
		if (serverResponses.size() != 1) {
			chain.throwException(
					"get download server error, illegal response",
					new IOException("get download server error, illegal response"));
			return null;
		}
		JdfsStatusResponse resp = (JdfsStatusResponse) serverResponses.get(0);
		if (resp.getStatus() != JdfsRequestConstants.STATUS_OK) {
			chain.throwException("get download server error", new IOException(
					"get download server error: " + resp.getMessage()));
			return null;
		} else {
