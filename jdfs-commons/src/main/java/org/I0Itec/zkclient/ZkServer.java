    ...
    private void startSingleZkServer(final int tickTime, final File dataDir, final File dataLogDir, final int port) {
        try {
           ...
           _zk.setMinSessionTimeout(_minSessionTimeout);
            //_nioFactory = new NIOServerCnxnFactory(new InetSocketAddress(port));
           _nioFactory = NIOServerCnxnFactory.createFactory(new InetSocketAddress(port), maxClientCnxns); 
