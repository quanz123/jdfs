...
public class FileRangeRequestMessageDecoder extends MessageDecoderAdapter {
	...
	public MessageDecoderResult decode(IoSession session, IoBuffer in,
		...
		// out.write(request);
		// return MessageDecoderResult.OK;
		DecoderState state = (DecoderState) session.getAttribute(RANGE_DATA);
		if(state == null){
			state = new DecoderState();
			session.setAttribute(RANGE_DATA, state);
		}
		if(state.request == null) {
			...
			long size = in.getLong();
			long position = in.getLong();
			UpdateFileDataRequest request = new UpdateFileDataRequest(id, size, position, null);
		...
		in.get(data);
		UpdateFileDataRequest request = state.request;
